...
TMP_ROOT = ROOT / "results/apexp_block/config_large_500/evaluation/final_evaluations/eagle3_rerun_tmp_7gpu"
LOG_ROOT = ROOT / "slurm_logs/rerun_eagle3_fixed_7gpu"
...
def copy_leaf_to_target(leaf: Path, target: Path, metadata: dict):
    ...
    if target.exists():
        backup = target.with_name(target.name + f"_BACKUP_before_eagle3_rerun_{int(time.time())}")
        if backup.exists():
...
def run_one_k(workload_cfg: dict, k: int, gpu_id: str):
    ...
    tmp_dir = TMP_ROOT / workload / "eagle3" / f"k{k}"
    ...
    log_file = LOG_ROOT / f"{workload}_eagle3_k{k}.log"
    ...
    # Keep offline because these models should already be cached.
    ...
    cmd = [
        ...
        "--workload", workload,
        "--run_name", f"{workload}_eagle3_k{k}_rerun",
    ...
    print(f"[START] workload={workload} k={k} gpu={gpu_id}", flush=True)
    ...
    metadata = {
        ...
        "note": "This exact eagle3 fixed-run directory was overwritten by the rerun.",
    }
...
def main():
    ...
    manifest_path = FIXED_RUNS_ROOT.parent / "eagle3_rerun_7gpu_manifest.json"
    ...
    if failed:
        raise RuntimeError(f"Some EAGLE3 workers failed: {failed}")

    print("[ALL DONE] EAGLE3 rerun completed for all workloads and k values.", flush=True)
